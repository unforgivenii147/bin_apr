...
class CommentRemover:
    ...
    def should_preserve_comment(self, comment_text: str) -> bool:
        ...
        if comment_text.startswith("#!"):
            return True
        return any(
            (pattern in comment_text for pattern in self.PRESERVE_PATTERNS))
...
def find_python_files(cwd: str = ".") -> list[str]:
    ...
    for py_file in glob.glob(os.path.join(cwd, "**", "*.py"), recursive=True):
        if any((part in py_file
                for part in ["__pycache__", ".git", ".venv", "venv", ".tox"])):
            continue
        python_files.append(py_file)
    return python_files


def process_files_mp(files: list[str],
                     num_workers: int | None = None) -> tuple[int, int]:
