...
def find_dist_info_dir(site_packages: Path, pkg_name: str) -> Path:
    """
    Find the first dist-info directory that seems to match the pkg_name.
    Example: for 'requests' finds 'requests-2.31.0.dist-info'.
    """
    candidates = list(site_packages.glob(f"{pkg_name}-*.dist-info"))
    if not candidates:
        # Some packages use underscores / normalized names
...
def copy_package_files(pkg_name: str):
    ...
    with record_path.open("r", encoding="utf-8", newline="") as f:
        reader = csv.reader(f)
        for row in reader:
            # RECORD format: path, hash (optional), size (optional)
            # Path is relative to site-packages (or may include .. in some cases)
            try:
                if not row:
                    continue
                rel_path = row[0]
                ...
                if not src_path.exists():
                    continue
                # Compute destination path, preserving structure under ~/tmp/1/<pkg_name>/
