...
def find_packages_with_bin_scripts(output_file="have_scripts.txt"):
    ...
    try:
        ...
        for i, package_name in enumerate(installed_packages):
            print(f"[{i + 1}/{total_packages}] Checking '{package_name}'...",
                  end="\r")
            try:
                ...
                for line in lines:
                    ...
                    for indicator in bin_indicators:
                        if (indicator in line.lower() and
                            (line.endswith(".py") or os.path.splitext(line)[1]
                             == "" or os.path.splitext(line)[1] == ".exe") and
                            (not any((exclude_part in line for exclude_part in [
                                "__pycache__", ".dist-info", ".egg-info", ".pth"
                            ])))):
