...
def extract_from_ast(code: str,
    ...
    try:
        ...
        for m in re.finditer(
                "(?:import_module|__import__)\\(\\s*['\\\"]([\\w\\.]+)['\\\"]\\s*\\)",
                code):
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            if isinstance(node.func, ast.Name) and node.func.id == "__import__":
                ...
                val = node.func
                if (isinstance(val.value, ast.Name) and
                        val.value.id == "importlib" and
                    (val.attr == "import_module")) and (
...
def build_project_module_map(sources: list[str]) -> dict[str, list[str]]:
    ...
    for fp in sources:
        ...
        if parts[-1] == "__init__.py":
            mod = ".".join(parts[:-1]) if parts[:-1] else parts[-2] if len(
                parts) > 1 else ""
        else:
...
def resolve_packages(imports: set[str], stdlib: set[str],
                     mapping: dict[str, str], pip_available: set[str],
                     project_toplevels: set[str]) -> set[str]:
    out = set()
    for imp in imports:
        if not imp:
            continue
...
def scan_sources(ignore_dirs: set[str]) -> list[str]:
    ...
    for root, dirs, files in os.walk("."):
        ...
        for f in files:
            ...
            lower = f.lower()
            if lower.endswith((".py", ".ipynb", ".whl", ".zip", ".tar.gz",
                               ".tgz", ".tar.xz")) or Path(fp).suffix == "":
                out.append(fp)
    return out
...
def main() -> None:
    ...
    cache = {} if args.no_cache else load_json(
    ...
    if tasks:
        with mp.Pool(mp.cpu_count()) as pool:
            for res in tqdm(pool.imap_unordered(process_raw, tasks),
                            total=len(tasks),
                            desc="Processing"):
    ...
    pkgs = resolve_packages(final_candidates, stdlib, mapping, piplist,
                            project_top_only)
