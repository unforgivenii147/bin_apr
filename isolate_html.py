...
def find_local_resource(resource_name, base_html_dir):
    ...
    for search_dir in search_paths:
        ...
        if resource_name.startswith("/"):
            path_stripped_slash = os.path.join(base_html_dir,
                                               resource_name.lstrip("/"))
    ...
    print(
        f"Resource '{resource_name}' not found in primary or fallback locations."
    )
    return None
    ...
    for img_tag in soup.find_all("img"):
        ...
        if src and (not src.startswith(("http://", "https://", "data:"))):
            ...
            if local_img_path:
                ...
                if encoded_img:
                    img_tag[
                        "src"] = f"data:{get_mime_type(local_img_path)};base64,{encoded_img}"
            else:
                print(
                    f"Warning: Image resource '{src}' not found, removing tag.")
                img_tag.decompose()
        ...
        if link_tag.get("rel") == ["stylesheet"]:
            ...
            if href and (not href.startswith(("http://", "https://", "data:"))):
                ...
                if local_css_path:
                    ...
                    try:
                        css_content = Path(local_css_path).read_text(
                            encoding="utf-8")
                        ...
                        print(
                            f"Error processing CSS file {local_css_path}: {e}")
                        link_tag.decompose()
                else:
                    print(
                    ...
                    if local_font_path:
                        ...
                        if encoded_font:
                            ...
                            style_content = re.sub(
                                re.escape(f"url({font_url})").replace(
                                    "\\(", "\\(").replace("\\)", "\\)"),
                                f"url('data:{mime_type};base64,{encoded_font}')",
                                style_content,
                                flags=re.IGNORECASE,
        ...
        if src and (not src.startswith(("http://", "https://", "data:"))):
            ...
            if local_script_path:
                try:
                    ...
                    print(
                        f"Error reading script content from {local_script_path}: {e}"
                    )
