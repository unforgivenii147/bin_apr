...
def copy_lines_to_clipboard(filename: str,
                            start_line: int,
...
def main():
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print(f"Usage: {sys.argv[0]} <filename> <start_line> [end_line]",
              file=sys.stderr)
        print("  <filename>: Path to the input file.", file=sys.stderr)
        print("  <start_line>: The first line number to copy (1-based index).",
              file=sys.stderr)
        print(
