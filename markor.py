...
class GUIFramework:
    ...
    def show_file_picker(self, initial_path: str | None = None) -> str | None:
        ...
        return input(
            f"Enter file path (starting from {initial_path}): ").strip() or None
...
class FileManager:
    ...
    def list_documents(self,
        ...
        } for file_path in search_path.glob(pattern) if file_path.is_file() and
                     file_path.suffix in {".md", ".txt", ".json"}]
...
class TextEditor:
    ...
    def close_document(self):
        if self.is_modified:
            save_choice = self.gui.show_dialog("Save Changes?",
