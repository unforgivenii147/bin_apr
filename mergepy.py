...
def resolve_imports(content, current_dir):
    ...
    content = re.sub("from \\. import ([a-zA-Z0-9_]+)",
                     f"from {folder_name} import \\1", content)
    content = re.sub("from \\.([a-zA-Z0-9_]+) import ([a-zA-Z0-9_]+)",
                     f"from {folder_name}.\\1 import \\2", content)
