...
def extract_mime_and_extensions(obj):
    """
    Recursively extract:
      - mime type from key "mime-type"
      - extensions from "glob" patterns like "*.xi"

    Returns:
      list[tuple[str, str]]  -> [(mime_type, ext), ...]
    """
    results = []

    if isinstance(obj, dict):
        # If this dict itself is a mime record
        if "mime-type" in obj:
            mt = obj["mime-type"]
...
def build_mime_to_ext(root_dir: Path) -> dict[str, list[str]]:
    ...
    for json_file in root_dir.rglob("*.json"):
        # Skip the output file if it already exists in the tree
        ...
        try:
            ...
            continue  # skip invalid/unreadable JSON files
