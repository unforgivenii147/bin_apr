...
REPLACEMENTS: dict[tuple[str, str], tuple[str | None, str, str]] = {
    ...
    ("os", "path.splitext"):
        ("pathlib", "Path", "lambda p: (Path(p).stem, Path(p).suffix)"),
    ("os", "path.splitext")[::-1]: None,
    ("os", "path.split"):
        ("pathlib", "Path", "lambda p: (str(Path(p).parent), Path(p).name)"),
    ("os", "path.getmtime"):
        ("pathlib", "Path", "lambda p: Path(p).stat().st_mtime"),
    ("os", "path.getsize"):
        ("pathlib", "Path", "lambda p: Path(p).stat().st_size"),
    ("os", "path.relpath"): (
        "pathlib",
        "Path",
    ...
    ("os", "path.commonpath"):
        ("pathlib", "Path", "lambda paths: Path(os.path.commonpath(paths))"),
    ("os", "path.samefile"):
        ("pathlib", "Path", "lambda p1, p2: Path(p1).samefile(Path(p2))"),
    ("os", "path.expanduser"):
        ("pathlib", "Path", "lambda p: Path(p).expanduser()"),
    ("os", "path.expandvars"):
        ("pathlib", "Path", "lambda p: Path(p).expandvars()"),
    ("os", "path.normpath"): ("pathlib", "Path", "lambda p: Path(p).resolve()"),
    ("os", "path.normcase"):
        ("pathlib", "Path", "lambda p: Path(p).resolve().as_posix()"),
    ("os", "makedirs"):
        ("shutil", "Path", "lambda p, *a, **k: Path(p).mkdir(*a, **k)"),
    ...
    ("os", "rename"):
        ("pathlib", "Path", "lambda src, dst: Path(src).rename(dst)"),
    ("os", "replace"):
        ("pathlib", "Path", "lambda src, dst: Path(src).replace(dst)"),
    ("os", "listdir"):
        ("pathlib", "Path", "lambda p='.': list(Path(p).iterdir())"),
    ("os", "walk"): (
        "pathlib",
        "Path",
    ...
    ("os", "chown"):
        ("pathlib", "Path", "lambda p, uid, gid: Path(p).chown(uid, gid)"),
    ("os", "symlink"):
        ("pathlib", "Path", "lambda src, dst: Path(dst).symlink_to(src)"),
    ("os", "readlink"):
        ("pathlib", "Path", "lambda p: str(Path(p).readlink())"),
...
class OsUsageFinder(ast.NodeVisitor):
    ...
    def visit_Attribute(self, node: ast.Attribute):
        if isinstance(node.value,
                      ast.Name) and node.value.id == self.os_import_name:
            self.uses_os = True
            self.os_used_attrs.add(node.attr)
        if isinstance(node.value, ast.Attribute) and (
                isinstance(node.value.value, ast.Name) and
                node.value.value.id == self.os_import_name and
...
def rewrite_os_to_pathlib(source: str, tree: ast.AST) -> str:
    source = re.sub("\\bos\\.path\\.join\\s*\\(\\s*([^)]*)\\s*\\)",
                    lambda m: _join_replacer(m.group(1)), source)
    source = re.sub("\\bos\\.getcwd\\s*\\(\\s*\\)", "Path.cwd()", source)
    source = re.sub("\\bos\\.listdir\\s*\\(\\s*\\)", "list(Path().iterdir())",
                    source)
    source = re.sub('\\bos\\.listdir\\s*\\(\\s*"([^"]+)"\\s*\\)',
                    'list(Path("\\1").iterdir())', source)
    source = re.sub("\\bos\\.listdir\\s*\\(\\s*\\'([^\\']+)\\'\\s*\\)",
                    'list(Path("\\1").iterdir())', source)
    for attr in [
            "exists",
                ...
                if os_attr[1] == "makedirs":
                    source = re.sub(
                        ...
                        (f"Path({m.group(1)}).mkdir(parents=True, exist_ok=True)"
                         if m.group(2) is None else
                         f"Path({m.group(1)}).mkdir({m.group(2)})"),
                        source,
                    )
                    ...
                    source = re.sub("\\bos\\.rmdir\\s*\\(\\s*([^)]+)\\s*\\)",
                                    "Path(\\1).rmdir()", source)
                elif os_attr[1] == "remove":
                    source = re.sub("\\bos\\.remove\\s*\\(\\s*([^)]+)\\s*\\)",
                                    "Path(\\1).unlink()", source)
                elif os_attr[1] == "rename":
                    source = re.sub(
                        "\\bos\\.rename\\s*\\(\\s*([^,]+)\\s*,\\s*([^)]+)\\s*\\)",
                        "Path(\\1).rename(\\2)", source)
                elif os_attr[1] == "replace":
                    source = re.sub(
                        "\\bos\\.replace\\s*\\(\\s*([^,]+)\\s*,\\s*([^)]+)\\s*\\)",
                        "Path(\\1).replace(\\2)", source)
                elif os_attr[1] == "stat":
                    source = re.sub("\\bos\\.stat\\s*\\(\\s*([^)]+)\\s*\\)",
                                    "Path(\\1).stat()", source)
                elif os_attr[1] == "chmod":
                    source = re.sub(
                        "\\bos\\.chmod\\s*\\(\\s*([^,]+)\\s*,\\s*([^)]+)\\s*\\)",
                        "Path(\\1).chmod(\\2)", source)
                    ...
                    source = re.sub(
                        ...
                        "Path(\\2).symlink_to(\\1)", source)
                elif os_attr[1] == "readlink":
                    source = re.sub("\\bos\\.readlink\\s*\\(\\s*([^)]+)\\s*\\)",
                                    "str(Path(\\1).readlink())", source)
                elif os_attr[1] == "unlink":
                    source = re.sub("\\bos\\.unlink\\s*\\(\\s*([^)]+)\\s*\\)",
                                    "Path(\\1).unlink()", source)
                elif os_attr[1] == "scandir":
                    source = re.sub("\\bos\\.scandir\\s*\\(\\s*([^)]*)\\s*\\)",
                                    "Path(\\1).iterdir()", source)
...
def _make_pathlib_call(attr: str) -> str:
    mapping = {
        "exists":
            lambda p: f"Path({p}).exists()",
        "isdir":
            lambda p: f"Path({p}).is_dir()",
        "isfile":
            lambda p: f"Path({p}).is_file()",
        "abspath":
            lambda p: f"Path({p}).resolve()",
        "realpath":
            lambda p: f"Path({p}).resolve()",
        "basename":
            lambda p: f"Path({p}).name",
        "dirname":
            lambda p: f"Path({p}).parent",
        "splitext":
            lambda p: f"(Path({p}).stem, Path({p}).suffix)",
        "split":
            lambda p: f"(str(Path({p}).parent), Path({p}).name)",
        "getmtime":
            lambda p: f"Path({p}).stat().st_mtime",
        "getsize":
            lambda p: f"Path({p}).stat().st_size",
        "relpath":
            lambda p: f"Path({p}).resolve().relative_to(Path('.').resolve())",
        "samefile":
            lambda p: f"Path({p}).exists() and Path({p}).samefile(Path({p}))",
        "expanduser":
            lambda p: f"Path({p}).expanduser()",
        "expandvars":
            lambda p: f"Path({p}).expandvars()",
        "normpath":
            lambda p: f"str(Path({p}).resolve())",
        "normcase":
            lambda p: f"Path({p}).as_posix().lower()",
    }
    ...
    for target in targets:
        ...
        if p.is_file() and p.suffix == ".py":
            ...
            for root, dirs, files in os.walk(p):
                ...
                py_files.extend(
                    (Path(root) / f for f in files if f.endswith(".py")))
