...
def format_python_file(filepath):
    ...
    try:
        with open(filepath, "r", encoding="utf-8") as f_in, open(backup_filepath, "w", encoding="utf-8") as f_bak:
            content = f_in.read()
            f_bak.write(content)
    except IOError as e:
        print(f"Error creating backup file {backup_filepath}: {e}", file=sys.stderr)
        return
    formatted_lines = []
    ...
    for i, line in enumerate(lines):
        ...
        if stripped_line.startswith(DOC_TH1) or stripped_line.startswith(DOC_TH2):
    ...
    try:
        ...
        try:
            with open(filepath, "w", encoding="utf-8") as f_out:
                f_out.write(final_formatted_content)
