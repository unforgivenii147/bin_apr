...
def process_file(pdf_path: Path):
    ...
    try:
        ...
        logger.info(f"Error creating output directory {output_folder}: {e}")
        return
    try:
        reader = PdfReader(pdf_path)
    except Exception as e:
        logger.info(f"Error opening PDF file {pdf_path}: {e}")
        return
    ...
    for page_num in range(num_pages):
        try:
            ...
            logger.info(f"Error processing page {page_num + 1}: {e}")
