...
cwd = Path.cwd()
IGNORED_DIRS = {
    ".git", ".hg", ".svn", "node_modules", "__pycache__", ".ruff_cache",
...
def matches_any_glob(path: Path, patterns: Iterable[str]) -> bool:
    basename = path.name
    return any((fnmatch.fnmatch(path, p) or fnmatch.fnmatch(basename, p)
                for p in patterns))
    ...
    try:
        with path.open(encoding="utf-8", errors="replace") as fh:
            for lineno, raw_line in enumerate(fh, start=1):
                ...
                if regex:
                    spans.extend(
                        ((m.start(), m.end()) for m in regex.finditer(line)))
                else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not pattern:
        print("No pattern provided. Use positional PATTERN or -e PATTERN.",
              file=sys.stderr)
        return 2
