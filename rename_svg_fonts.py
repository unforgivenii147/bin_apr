...
def extract_font_id(svg_text):
    """
    Extracts font id="..." from <font ...> tag.
    Returns the id string, or None if not found.
    """
    # Regex captures: <font id="something" ... >
...
def rename_svg_font(file_path_obj):
    """
    Renames a single SVG file based on its font ID.
    `file_path_obj` should be a Path object.
    """
    ...
    # Sanitize font_id for use as a filename if necessary (basic example)
    # This is a more robust way to handle potential invalid characters in IDs
    ...
    if sanitized_font_id != font_id:
        ...
        font_id = sanitized_font_id  # Use sanitized ID

    # new filename: <font_id>.svg
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        # Process a single file provided as argument
        ...
        # Process current directory if no argument is given
        current_dir = Path(".")
