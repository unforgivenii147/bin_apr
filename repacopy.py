...
class PackageRepacker:
    ...
    def find_site_packages_dirs(self) -> list[Path]:
        ...
        for search_path in search_paths:
            ...
            try:
                ...
                logger.debug("Permission denied scanning %s: %s", search_path,
                             e)
    ...
    def get_package_info_from_dist_info(self, dist_info_dir: Path) -> dict:
        ...
        if metadata_file.exists():
            try:
                ...
                logger.warning("Could not read metadata from %s: %s",
                               metadata_file, e)
        return metadata
    ...
    def create_wheel_structure(
        ...
        if is_pure_python:
            ...
            try:
                ...
                logger.debug("Using 'packaging' lib. Tags: %s-%s-%s",
                ...
                logger.debug("Fallback tags: %s-%s-%s", python_tag, abi_tag,
                             platform_tag)
        ...
        if not new_metadata_path.exists() and original_metadata.exists():
            shutil.copy2(original_metadata, new_metadata_path)
        ...
        if original_record.exists():
            shutil.copy2(original_record, dist_info_dir / "RECORD")
        else:
            logger.warning("Could not find original RECORD file at %s",
                           original_record)
        return wheel_dir

    def copy_package_files(self, dist_info_dir: Path, site_packages_path: Path,
        ...
        try:
            ...
            if not package_name:
                logger.warning("Could not determine package name for %s",
                               dist_info_dir)
                return False
            files_to_include = []
            is_pure_python = True
            with Path(record_file).open(encoding="utf-8") as f:
                for line in f:
                    ...
                    if file_path_str and (
                            not file_path_str.endswith(".dist-info/RECORD")):
            ...
            package_structure_path = self.create_wheel_structure(
                package_name, metadata, files_to_include, site_packages_path,
                output_dir, dist_info_dir, is_pure_python)
            ...
            logger.exception(
                f"Error copying files for {dist_info_dir.name}: {e}")
            return False
    ...
    def copy_all_packages(self):
        ...
        for site_packages_dir in self.found_site_packages:
            ...
            env_name = "local_env"
            try:
                if (".venv" in str(site_packages_dir) or
            ...
            for dist_info_dir in dist_info_dirs:
                if dist_info_dir.is_dir() and self.copy_package_files(
                        dist_info_dir, site_packages_dir, output_dir):
                    package_count += 1
                    total_copied += 1
            print("Copied %s packages from %s", package_count,
                  site_packages_dir)
