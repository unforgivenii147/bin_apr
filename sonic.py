...
class FileSorter(LineProcessor):
    ...
    def process_file(
        ...
        try:
            ...
            self.log(
                f"Original file: {original_lines} lines, {self.fsz(original_size)}"
            )
            lines = list(
            ...
            return {
                ...
                "size_reduction_pct": (original_size - after) / original_size *
    ...
    def print_stats(self, stats: dict):
        ...
        if stats["duplicate_lines"] > 0:
            dup_pct = stats["duplicate_lines"] / stats[
                "original_lines"] * 100 if stats["original_lines"] > 0 else 0
            print(
        ...
        if stats["size_reduction"] > 0:
            print(
                f"Size reduction: {self.fsz(stats['size_reduction'])} ({stats['size_reduction_pct']:.1f}%)"
            )
        print()
...
class FileAnalyzer(LineProcessor):
    ...
    def analyze_file(self, file_path: Path, encoding: str = "utf-8") -> dict:
        ...
        avg_length = sum(
            (len(line) for line in lines)) / len(lines) if lines else 0
