...
def replace_tabs_in_file(file_path: Path):
    try:
        ...
        print(f"Error: Could not decode {file_path.name} with UTF-8. Skipping.",
              file=sys.stderr)
        return
    except OSError as e:
    ...
    try:
        ...
        print(f"Error writing to {file_path.name}: {e}. Skipping.",
              file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description=
        "Replaces tab characters with 4 spaces in Python files or specified text files."
    )
    parser.add_argument(
        "files",
        nargs="*",
        help=
        "Specific files to process. If none, all *.py files in current directory and subdirectories are processed.",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    if args.files:
        ...
        for f_path in file_paths:
            if not f_path.exists():
                print(f"Warning: File '{f_path}' not found. Skipping.",
                      file=sys.stderr)
